...
\section{Evaluation}
We ran SentinelAPI against the sandbox API, which seeds six vulnerabilities and also
exposes a \emph{secure} control endpoint (\texttt{/posts/\{id\}}, which correctly enforces
ownership and returns only declared fields). The scanner detected all six seeded findings
and produced \emph{zero} findings on the secure control, yielding
...
\begin{tabular}{lll}
\toprule
\textbf{Severity} & \textbf{Finding} & \textbf{Evidence} \\
\midrule
CRITICAL & BOLA/IDOR on \texttt{GET /users/\{id\}} & $A$'s token reads $B$'s record \\
CRITICAL & BOLA/IDOR on \texttt{GET /orders/\{id\}} & $A$'s token reads $B$'s order \\
HIGH & Exposure: \texttt{password\_hash}, \texttt{ssn}, \texttt{token} & absent from schema \\
HIGH & Exposure: \texttt{card\_last4} & absent from schema \\
\bottomrule
\end{tabular}
\caption{Detected findings. Reproductions: \texttt{curl -H "Authorization: Bearer tok\_A" .../users/4} returns \texttt{bob}'s record including hash, SSN, and token.}
\label{tab:findings}
\end{table}
